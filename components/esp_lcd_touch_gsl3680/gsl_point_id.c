  ...
  for (i = 0; i < POINT_MAX; i++) {
    ...
    for (j = i + 1; j < POINT_MAX; j++) {
      ...
      if (x > x_min && x < x_max && y > y_min && y < y_max) {
        point_now[i].other.x =
            (point_now[i].other.x + point_now[j].other.x + 1) / 2;
        point_now[i].other.y =
            (point_now[i].other.y + point_now[j].other.y + 1) / 2;
        point_now[j].all = 0;
        pressure_now[i] = pressure_now[i] > pressure_now[j] ? pressure_now[i]
                                                            : pressure_now[j];
        pressure_now[j] = 0;
        i--;
        point_near = 8;
...
static unsigned int CC128(unsigned int x, unsigned int **coe, int k) {
  if (k == 3) {
    ...
    if (x & 128)
      return (x & ~127) + 127 -
             (coe[(((127 - (x & 127)) >> 6) & 1) ^ 1][(127 - (x & 127)) & 63] &
              127);
    else
      return (x & ~127) + (coe[((x >> 6) & 1) ^ 1][x & 63] & 127);
  }
  ...
  for (i = 0; i < (int)point_num && i < POINT_MAX; i++) {
    ...
    if (point_now[i].other.x >= edge_size &&
      ...
      if (global_state.other.active) {
        point_now[i].other.x =
            CCO(point_now[i].other.x, multi_group[LINE_SIZE - 2], 2);
    ...
    if (point_now[i].other.y >= edge_size &&
      ...
      if (global_state.other.active) {
        point_now[i].other.y =
            CCO(point_now[i].other.y, multi_group[LINE_SIZE - 1], 2);
  ...
  for (i = 0; i < POINT_MAX; i++) {
    if (pp[1][i].all != 0) {
      if (global_state.other.interpolation != 0 &&
          global_state.other.interpolation != INTE_INIT && pp[3][i].all &&
          pp[3][i].other.fill == 0) {
        if (pp[4][i].all && pp[5][i].all && pp[5][i].other.fill == 0)
          PointPredictD3(i);
        else
          PointPredictD2(i);
      } else if (global_state.other.interpolation || pp[2][i].all == 0 ||
                 pp[2][i].other.fill != 0 || pp[3][i].other.fill != 0 ||
                 pp[1][i].other.key != 0 || global_state.other.only) {
        PointPredictOne(i);
  ...
  if (global_state.other.only || global_state.other.active) {
    do {
      if (DistanceMin(&distance)) {
        if (pp[1][0].all != 0 &&
            pp[1][0].other.key != point_now[distance.j].other.key) {
...
static void PointOrder(void) {
  ...
  for (i = 0; i < POINT_MAX; i++) {
    if (pp[0][i].other.fill == 0)
      continue;
    if (pp[1][i].all == 0 || pp[1][i].other.fill != 0 || filter_able == 0 ||
        filter_able == 1) {
      pp[0][i].all = 0;
      pressure_now[i] = 0;
    }
...
static void PointCross(void) {
  ...
  for (j = 0; j < POINT_MAX; j++) {
    for (i = j + 1; i < POINT_MAX; i++) {
      if (pp[0][i].all == 0 || pp[0][j].all == 0 || pp[1][i].all == 0 ||
          pp[1][j].all == 0)
        continue;
  ...
  for (i = 0; i < POINT_MAX; i++) {
    ...
    if (pp[0][i].all != 0 && point_delay[i].other.init == 0 &&
      ...
      if (delay_key && pp[0][i].other.key) {
        point_delay[i].other.delay =
            (delay_key >> 3 * ((point_num > 10 ? 10 : point_num) - 1)) & 0x7;
        point_delay[i].other.report = 0;
        point_delay[i].other.dele = 0;
      } else {
        point_delay[i].other.delay =
            (report_delay >> 3 * ((point_num > 10 ? 10 : point_num) - 1)) & 0x7;
        point_delay[i].other.report =
            (report_ahead >> 3 * ((point_num > 10 ? 10 : point_num) - 1)) & 0x7;
        point_delay[i].other.dele =
            (report_delete >> 3 * ((point_num > 10 ? 10 : point_num) - 1)) &
            0x7;
        if (point_delay[i].other.report > point_delay[i].other.delay)
          point_delay[i].other.report = point_delay[i].other.delay;
        point_delay[i].other.report =
            point_delay[i].other.delay - point_delay[i].other.report;
    ...
    if (point_delay[i].other.able == 0 && point_delay[i].other.init != 0) {
      for (j = 0; j <= (int)point_delay[i].other.delay; j++) {
        if (pp[j][i].all == 0 || pp[j][i].other.fill != 0 ||
            pp[j][i].other.able != 0)
          break;
      }
...
static void PointFilter(void) {
  ...
  if (filter_able > 1) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
      ...
      FilterOne(i, ps_coe[filter_deep[i]], pr_coe[filter_deep[i]],
                0 - filter_able);
    }
  }
}
...
static void PointRound(void) {
  ...
  for (i = 0; i < 4; i++) {
    if (stretch->up[i].range)
      stretch->up[i].range = stretch->up[i].range * sen_num_nokey *
                             drv_num_nokey * 64 / screen_x_max;
    if (stretch->down[i].range)
      stretch->down[i].range = stretch->down[i].range * sen_num_nokey *
                               drv_num_nokey * 64 / screen_x_max;
    ...
    if (stretch->right[i].range)
      stretch->right[i].range = stretch->right[i].range * sen_num_nokey *
                                drv_num_nokey * 64 / screen_y_max;
  }
  ...
  for (id = 0; id < POINT_MAX; id++) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      coe[i] = 0;
      if (x < x0) {
        r[i] += (x0 - x) * stretch->up[i].range / dis * (x0 - x) *
                stretch->up[i].range / dis;
        coe[i] += (x0 - x) * stretch->up[i].coe / dis * (x0 - x) *
                  stretch->up[i].coe / dis;
      }
      if (x > x0) {
        r[i] += (x - x0) * stretch->down[i].range / dis * (x - x0) *
                stretch->down[i].range / dis;
        coe[i] += (x - x0) * stretch->down[i].coe / dis * (x - x0) *
                  stretch->down[i].coe / dis;
      }
      if (y < y0) {
        r[i] += (y0 - y) * stretch->left[i].range / dis * (y0 - y) *
                stretch->left[i].range / dis;
        coe[i] += (y0 - y) * stretch->left[i].coe / dis * (y0 - y) *
                  stretch->left[i].coe / dis;
      }
      if (y > y0) {
        r[i] += (y - y0) * stretch->right[i].range / dis * (y - y0) *
                stretch->right[i].range / dis;
        coe[i] += (y - y0) * stretch->right[i].coe / dis * (y - y0) *
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (stretch->right[i].range > screen_y_max * 64 / 128 ||
      ...
      for (i = 0; i < 4; i++) {
        if (stretch->up[i].range)
          stretch->up[i].range =
              stretch->up[i].range * drv_num_nokey * 64 / screen_x_max;
        if (stretch->down[i].range)
          stretch->down[i].range = (screen_x_max - stretch->down[i].range) *
                                   drv_num_nokey * 64 / screen_x_max;
        if (stretch->left[i].range)
          stretch->left[i].range =
              stretch->left[i].range * sen_num_nokey * 64 / screen_y_max;
        if (stretch->right[i].range)
  ...
  for (id = 0; id < POINT_MAX; id++) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (data[1] < stretch->left[i].range) {
        data[0] +=
            (stretch->left[i].range - data[1]) * stretch->left[i].coe / 128;
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (data[1] < stretch->right[i].range) {
        data[0] +=
            (stretch->right[i].range - data[1]) * stretch->right[i].coe / 128;
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (data[1] < stretch->down[i].range) {
        data[0] +=
            (stretch->down[i].range - data[1]) * stretch->down[i].coe / 128;
      ...
      if (dn < ds[0]) {
        for (j = 0; j <= len; j++) {
          if (j == len || dn == 0) {
            ...
            dr = dc[j + 1] +
                 ((dn - ds[j + 1]) * (dc[j] - dc[j + 1])) / (ds[j] - ds[j + 1]);
            ps[0][i].other.x =
                (int)ps[1][i].other.x +
                (((int)pp[0][i].other.x - (int)ps[1][i].other.x) * dr + 64) /
                    128;
            break;
          }
        }
      ...
      if (dn < ds[0]) {
        for (j = 0; j <= len; j++) {
          if (j == len || dn == 0) {
            ...
            ps[0][i].other.y =
                (int)ps[1][i].other.y +
                (((int)pp[0][i].other.y - (int)ps[1][i].other.y) * dr + 64) /
                    128;
            break;
          }
        }
  ...
  for (i = 0; i < POINT_MAX; i++) {
    ...
    if (i == 0) {
      ...
      if (point_edge.rate != 0 && point_edge.dis != 0) {
        ...
        if (temp >= point_edge.dis * edge_first_coe / 0x80) {
          ...
          temp = (point_edge.dis * edge_first_coe / 0x80 - temp) * 0x1000 /
                 point_edge.dis;
    ...
    if (ps[1][i].all == 0) {
      continue;
               ...
               pp[0][i].all && point_delay[i].other.able == 0 &&
               shake_min != 0) {
      dn = 0;
  ...
  if (len == 1) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
      ...
      if (dn == 0 || dr == 0) {
        ...
        ps[0][i].other.x =
            (int)ps[1][i].other.x +
            ((int)pp[0][i].other.x - (int)ps[1][i].other.x) * dr / dn;
        ps[0][i].other.y =
            (int)ps[1][i].other.y +
            ((int)pp[0][i].other.y - (int)ps[1][i].other.y) * dr / dn;
      }
      if (dc[0] > 0) {
    ...
    for (i = 0; i < 8 && i < len; i++) {
      if (stretch_mult)
        ds[i + 1] = shake_all[i].dis *
                    (stretch_mult * (temp > 1 ? temp - 1 : 0) + 0x80) / 0x80;
      else
        ds[i + 1] = shake_all[i].dis;
  ...
  for (i = 0; i < POINT_MAX; i++) {
    ...
    if (point_corner & (0x2 << i * 2)) {
      ...
      if (dis <= diagonal_size * 4) {
        DiagonalCompress(&(ps[0][i]), point_corner & (0x1 << i * 2), dis,
                         diagonal_size);
      ...
      if (pr[0][0].other.x < point_press_move.other.x &&
          pr[0][0].other.y <
              point_press_move.other.y +
                  (point_press_move.other.x - pr[0][0].other.x) &&
          pr[0][0].other.y + (point_press_move.other.x - pr[0][0].other.x) >
              point_press_move.other.y)
        point_press_move.all = 1;
      else if (pr[0][0].other.x > point_press_move.other.x &&
               pr[0][0].other.y <
                   point_press_move.other.y +
                       (pr[0][0].other.x - point_press_move.other.x) &&
               pr[0][0].other.y +
                       (pr[0][0].other.x - point_press_move.other.x) >
                   point_press_move.other.y)
        point_press_move.all = 2;
               ...
               pr[0][0].other.x <
                   point_press_move.other.x +
                       (point_press_move.other.y - pr[0][0].other.y) &&
               pr[0][0].other.x +
                       (point_press_move.other.y - pr[0][0].other.y) >
                   point_press_move.other.x)
        point_press_move.all = 3;
               ...
               pr[0][0].other.x <
                   point_press_move.other.x +
                       (pr[0][0].other.y - point_press_move.other.y) &&
               pr[0][0].other.x +
                       (pr[0][0].other.y - point_press_move.other.y) >
                   point_press_move.other.x)
...
void gsl_DataInit(unsigned int *conf_in) {
  ...
  sen_num = 10;
  drv_num_nokey = 16;
  sen_num_nokey = 128; // Suficiente para cubrir 1280 (128*10)
  for (i = 0; i < 4; i++)
    edge_cut[i] = 0;
  for (i = 0; i < 32; i++)
    stretch_array[i] = 0;
  ...
  for (i = 0; i < 8 * 3; i++)
    key_range_array[i] = 0;
  filter_able = 0;
  filter_coe[0] = (0 << 6 * 4) + (0 << 6 * 3) + (0 << 6 * 2) + (40 << 6 * 1) +
                  (24 << 6 * 0);
  filter_coe[1] = (0 << 6 * 4) + (0 << 6 * 3) + (16 << 6 * 2) + (24 << 6 * 1) +
                  (24 << 6 * 0);
  filter_coe[2] = (0 << 6 * 4) + (16 << 6 * 3) + (24 << 6 * 2) + (16 << 6 * 1) +
                  (8 << 6 * 0);
  filter_coe[3] = (6 << 6 * 4) + (16 << 6 * 3) + (24 << 6 * 2) + (12 << 6 * 1) +
                  (6 << 6 * 0);
  for (i = 0; i < 4; i++) {
    multi_x_array[i] = 0;
    multi_y_array[i] = 0;
  ...
  if (conf_in[0] <= 0xfff) {
    ...
    for (i = 0; i < 64; i++) {
      ...
      coordinate_correct_coe_y[i] =
          (conf[0x109 + 64 / 4 + i / 4] >> (i % 4 * 8)) & 0xff;
    }
    for (j = 0; j < 4; j++)
      for (i = 0; i < 64; i++)
        multi_group[j][i] = (conf[0x109 + 64 / 4 * 2 + (i + j * 64) / 4] >>
                             ((i + j * 64) % 4 * 8)) &
                            0xff;
