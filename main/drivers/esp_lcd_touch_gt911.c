...
static esp_err_t esp_lcd_touch_gt911_get_track_id(esp_lcd_touch_handle_t tp,
                                                  uint8_t *track_id,
                                                  uint8_t point_num);
...
esp_err_t esp_lcd_touch_new_i2c_gt911(const esp_lcd_panel_io_handle_t io,
  ...
  if (gt911_config && esp_lcd_touch_gt911->config.rst_gpio_num != GPIO_NUM_NC &&
    ...
    ESP_RETURN_ON_ERROR(
        gpio_set_level(esp_lcd_touch_gt911->config.rst_gpio_num,
                       esp_lcd_touch_gt911->config.levels.reset),
        TAG, "GPIO set level error!");
    ESP_RETURN_ON_ERROR(
        gpio_set_level(esp_lcd_touch_gt911->config.int_gpio_num, 0), TAG,
    ...
    ESP_LOGI(TAG, "I2C address initialization procedure skipped - using "
                  "default GT9xx setup");
    /* Reset controller */
  ...
  if ((buf[0] & 0x80) == 0x00) {
    ...
    uint8_t key_max =
        ((ESP_GT911_TOUCH_MAX_BUTTONS < CONFIG_ESP_LCD_TOUCH_MAX_BUTTONS)
             ...
             : (CONFIG_ESP_LCD_TOUCH_MAX_BUTTONS));
    ...
    touch_cnt = (touch_cnt > CONFIG_ESP_LCD_TOUCH_MAX_POINTS
...
static bool esp_lcd_touch_gt911_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x,
                                       ...
                                       uint8_t *point_num,
                                       uint8_t max_point_num) {
  ESP_RETURN_ON_FALSE(tp != NULL, false, TAG,
                      "Touch controller handle can't be NULL");
  ESP_RETURN_ON_FALSE(x != NULL, false, TAG,
                      "Pointer to the x coordinates array can't be NULL");
  ESP_RETURN_ON_FALSE(y != NULL, false, TAG,
                      "Pointer to the y coordinates array can't be NULL");
  ESP_RETURN_ON_FALSE(point_num != NULL, false, TAG,
                      "Pointer to number of touch points can't be NULL");
  ESP_RETURN_ON_FALSE(max_point_num > 0, false, TAG,
                      "Array size must be equal or larger than 1");
...
static esp_err_t esp_lcd_touch_gt911_get_button_state(esp_lcd_touch_handle_t tp,
  ...
  ESP_RETURN_ON_FALSE(state != NULL, ESP_ERR_INVALID_ARG, TAG,
                      "Pointer to the state array can't be NULL");
